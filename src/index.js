...
const REGISTER_COMMANDS_ON_START = process.env.REGISTER_COMMANDS_ON_START !== 'false';
...
function printAuthHelp() {
  console.error('Discord API authentication failed (401 Unauthorized).');
  console.error('Check these values and restart the app:');
  console.error('- DISCORD_TOKEN: bot token from Discord Developer Portal > Bot > Reset Token');
  console.error('- DISCORD_CLIENT_ID: application ID from General Information');
  console.error('- DISCORD_GUILD_ID: your server ID (Developer Mode)');
  console.error('If commands are already registered, you can temporarily set REGISTER_COMMANDS_ON_START=false.');
}
...
async function startup() {
  if (REGISTER_COMMANDS_ON_START) {
    ...
    console.log('Skipping slash command registration (REGISTER_COMMANDS_ON_START=false).');
  }
  ...
  try {
    ...
    if (error && error.code === 'TokenInvalid') {
      console.error('Discord login failed: TokenInvalid. Verify DISCORD_TOKEN and restart the app.');
    } else {
